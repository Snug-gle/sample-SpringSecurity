...
public class LoginController {
    ...
    // 로그인 화면 요청 처리
    @GetMapping("/login")
    public String loginForm(@ModelAttribute("loginForm") UserVo userVo) {
        return "loginForm";
    }

    @PostMapping("/login")
    public String login(@ModelAttribute UserVo userVo, BindingResult bindingResult, HttpServletRequest request) {
        ...
        if (loginUser == null) {
            ...
            System.out.println("아이디 비번 메시지 알림");
            return "loginForm";
        }

        // 로그인 성공 시 세션이 있으면 있는 세션에 반환, 없으면 새로 생성
...
//
//        // 세션에 로그인 회원 정보 보관함
//        session.setAttribute(SessionConst.LOGIN_USER, loginUser.getName());
//
//        return "redirect:/";
//    }
}
